...
export function playSessionAmbientAudio(delaySeconds = 0, customAudioSourceUrl = null) {
    const AudioContext = window.AudioContext || window.webkitAudioContext;
    ...
    if (customAudioSourceUrl) {
        // If a custom audio file is provided, use an audio element to play it
        if (window.__customAudioElement) {
            window.__customAudioElement.pause();
            window.__customAudioElement.removeAttribute('src');
            window.__customAudioElement.load();
        }

        window.__customAudioElement = new Audio();
        window.__customAudioElement.loop = true;
        window.__customAudioElement.src = customAudioSourceUrl;

        // We need to create a MediaElementSource and connect it to our gain node
        // Create a new one every time to ensure it binds to the current activeSessionAudioCtx
        const sourceNode = activeSessionAudioCtx.createMediaElementSource(window.__customAudioElement);
        sourceNode.connect(activeSessionGainNode);

        // Start playing the element
        window.__customAudioElement.play().catch(e => console.warn("Custom audio play blocked:", e));

        // Skip playing the drone sound and just play the bowl chimes
    } else {
...
export function stopSessionAmbientAudio(immediate = false) {
    ...
    if (activeSessionAudioCtx && activeSessionGainNode) {
        ...
        // Clean up custom audio if playing
        if (window.__customAudioElement) {
            window.__customAudioElement.pause();
            window.__customAudioElement.currentTime = 0;
            // Clean up old object url to prevent memory leaks if new one selected later
            if (window.__customAudioElement.src.startsWith('blob:')) {
                URL.revokeObjectURL(window.__customAudioElement.src);
            }
            window.__customAudioElement.src = "";
        }
