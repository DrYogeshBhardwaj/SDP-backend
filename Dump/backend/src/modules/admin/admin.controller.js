...
const getDashboardStats = async (req, res) => {
    try {
        ...
        return successResponse(res, 200, 'Dashboard stats retrieved', {
            ...
            totalIncomePaid: totalIncomePaid._sum.amount || 0,
            pendingPayoutAmount: pendingPayoutAmount._sum.amount || 0,
            totalPayoutApproved: totalPayoutApproved._sum.amount || 0,
            todayTherapyMinutes: todayTherapyMinutes._sum.amount || 0,
            activeSessions,
            todayDemos,
            systemWallet: systemWallet || 0
        });
    } catch (error) {
...
const updatePayoutStatus = async (req, res) => {
    try {
        ...
        if (status === 'REJECTED' && (!remarks || remarks.length < 5)) {
            return errorResponse(res, 400, 'Reason is mandatory for rejection (min 5 chars)');
        }

        // 1. ATOMIC SNAPSHOT & IDEMPOTENCY CHECK
        return await prisma.$transaction(async (tx) => {
            ...
            // IDEMPOTENCY: Block if already PAID or REJECTED
            if (payout.payoutStatus === 'PAID' || payout.payoutStatus === 'REJECTED') {
                throw new Error(`Cannot update payout in ${payout.payoutStatus} state.`);
            }

            // IDEMPOTENCY: Block duplicate APPROVED calls
            if (status === 'APPROVED' && payout.payoutStatus === 'APPROVED') {
            ...
            // 2. FREEZE SNAPSHOT (On Approval)
            if (status === 'APPROVED') {
                const { getDerivedBalance, getSystemWallet } = require('../finance/finance.service');
                updateData.wallet_before = await getDerivedBalance(payout.userId);
                updateData.system_wallet_snapshot = await getSystemWallet();
            }

            // 3. FINAL SNAPSHOT (On Paid)
            if (status === 'PAID') {
                const { getDerivedBalance } = require('../finance/finance.service');
                updateData.wallet_after = await getDerivedBalance(payout.userId);
            }
            ...
            if (txn) {
                await tx.transaction.update({
                    ...
                    data: {
                        ...
                        description: `Payout Action: ${status} by Admin`
                    }
                });
            }
            ...
            await logAdminAction(adminId, `PAYOUT_${status}`, payout.userId, `Payout ${id} processed as ${status}`);

            return successResponse(res, 200, `Payout successfully marked as ${status}`);
        });
    } catch (error) {
        console.error('Update Payout Error:', error.message);
        return errorResponse(res, 400, error.message || 'Failed to update payout status');
    }
};
...
const getRiskAlerts = async (req, res) => {
    try {
        const [duplicateUPIs, highIntensity] = await Promise.all([
            // Duplicate UPI Check
            prisma.user.groupBy({
                by: ['upi_id'],
                where: { upi_id: { not: null } },
                _count: { mobile: true },
                having: { mobile: { _count: { gt: 1 } } }
            }),
            // High Intensity (10+ joins in 24h)
            prisma.user.groupBy({
                by: ['sponsor_id'],
                where: { 
                    sponsor_id: { not: null },
        ...
        return successResponse(res, 200, 'Risk alerts retrieved', {
            duplicateUPIs,
            highIntensity
        });
    } catch (error) {
        return errorResponse(res, 500, 'Failed to fetch risk alerts');
    }
};
...
const getSystemSettings = async (req, res) => {
    try {
        ...
        return successResponse(res, 200, 'Settings retrieved', config);
    } catch (error) {
...
const updateSystemSetting = async (req, res) => {
    try {
        ...
        await logAdminAction(req.user.id, 'SETTING_CHANGE', null, `Setting ${key} changed to ${value}`);
        return successResponse(res, 200, 'Setting updated');
    } catch (error) {
        return errorResponse(res, 500, 'Failed to update setting');
    }
};
