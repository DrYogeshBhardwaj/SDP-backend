...
const purgeRevenue = async (req, res) => {
    try {
        console.log(`[PURGE_START] Requested by admin ${req.user.userId}`);
        ...
        return successResponse(res, 200, 'Revenue Reset Successful', { created, target: 15 });
    } catch (err) {
        return errorResponse(res, 500, 'Purge failed: ' + err.message);
    }
};
