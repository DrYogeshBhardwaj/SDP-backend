...
const requestPayout = async (req, res) => {
    try {
        // 0. CHECK SAFETY TOGGLE
        const payoutsEnabled = await prisma.systemSetting.findUnique({ where: { key: 'PAYOUTS_ENABLED' } });
        if (payoutsEnabled && payoutsEnabled.value === 'false') {
            return errorResponse(res, 403, 'System Payouts are currently paused for maintenance.');
        }
