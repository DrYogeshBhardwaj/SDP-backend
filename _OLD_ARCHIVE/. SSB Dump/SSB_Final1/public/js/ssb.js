...
const SSBLogic = {
    ...
    startDemo: function () {
        ...
        const demoSession = {
            ...
            name: 'Guest User',
            kitId: 'KIT1',
            isPaid: true, // Demo is always paid/active
            wallet: 10,   // Free minutes
            consumed: 0,
            slots: [],
            deviceId: 'DEMO_DEVICE_' + Date.now(),
            date: Date.now(),
            mixColor: 'radial-gradient(circle, #00f0ff, #000)',
            left: { note: 417 },
            right: { note: 528 }
        };
    ...
    markPaid: function (kitId = 'KIT1') {
        ...
        if (data) {
            ...
            this.updateHistory(data.phone, {
                ...
                slots: currentSlots,
                name: data.name,
                referrer: referrer // Fix: Save Referrer in Primary Record
            });
        }
    },
    ...
    creditSale: function (referrerMobile, kitId = 'KIT1') {
        ...
        // GLOBAL SALES LOG (For Admin) - FIX: Use updateHistory to avoid duplicates
        if (session && session.phone) {
            this.updateHistory(session.phone, {
                kitId: kitId,
                referrer: referrerMobile,
                date: Date.now() // Update timestamp to latest
            });
        }
    },
    ...
    registerSeller: function (mobile, name, city, pincode) {
        ...
        if (!sellers[mobile]) {
            ...
            console.log("Seller Registered:", mobile);
        }

        // 2. Initialize Partner Stats (For Admin 'Data' Tab)
        let stats = localStorage.getItem('ssb_partner_stats');
        stats = stats ? JSON.parse(stats) : {};
        if (!stats[mobile]) {
            stats[mobile] = { sales: 0, earnings: 0, buyers: [] };
            localStorage.setItem('ssb_partner_stats', JSON.stringify(stats));
        }

        // 3. Force Admin UI Refresh if open in another tab
        // (Optional, mostly relies on reload)
    },
    ...
    isAuthorizedSeller: function (mobile) {
        ...
        // Manual Override for Admin/Testing/Demo + VIP User
        if (['9211755210', '8851168290', '9999999991', '8178591586'].includes(mobile)) return true;
    ...
    verifyPin: function (mobile, pin) {
        ...
        const protectedNumbers = ['9818869428', '8851168290', '9999999991', '8178591586'];
        if (protectedNumbers.includes(mobile)) {
            return pin === '1234';
        }

        // 4. NEW USER REGISTRATION (Auto-Set PIN)
        // If we reached here, the user has NO PIN set.
        // We save this PIN as their password for future.
        db[mobile] = pin;
        localStorage.setItem('ssb_auth_pins', JSON.stringify(db));
    ...
    hasAuthPin: function (mobile) {
        ...
        const protectedNumbers = ['9818869428', '8851168290', '8178591586'];
        if (protectedNumbers.includes(mobile)) return true;

        return false;
    },

    ensureCriticalAccounts: function () {
        // FOUNDER
        const founderMobile = '8851168290';
        const founderName = 'Poonam Sharma';

        // VIP USER (Requested)
        const vipMobile = '8178591586';
        const vipName = 'Authorized Seeder';
        ...
        if (!sellers[founderMobile] || sellers[founderMobile].status !== 'ACTIVE') {
            sellers[founderMobile] = { name: founderName, mobile: founderMobile, city: 'Delhi', pincode: '110001', joined: Date.now(), upline: null, status: 'ACTIVE', pin: '1234', sellerId: 'S1001' };
            modified = true;
        }

        // Restore VIP
        if (!sellers[vipMobile]) {
            sellers[vipMobile] = { name: vipName, mobile: vipMobile, city: 'Delhi', pincode: '110001', joined: Date.now(), upline: null, status: 'ACTIVE', pin: '1234', sellerId: 'S1002' };
            modified = true;
        }

        if (modified) {
            localStorage.setItem('ssb_authorized_sellers', JSON.stringify(sellers));
            console.log("Critical Accounts Restored");
        }

        // Ensure SMP Status (1st Active SMP)
        ...
        if (!smpList[founderMobile] || smpList[founderMobile].status !== 'ACTIVE') {
            smpList[founderMobile] = { mobile: founderMobile, name: founderName, address: 'Founder Office', pincode: '110001', aadhar: 'XXXX', photo: 'images/poonam_sharma.jpg', date: Date.now(), status: 'ACTIVE', approvedDate: Date.now() };
    ...
    // NEW: Fixed Alarm Logic
    startFixedWeeklyReminder: function (times) {
        if (!times || !Array.isArray(times)) times = [];

        // Save Config
        const config = { times: times, enabled: times.length > 0 };
        localStorage.setItem('ssb_reminder_config', JSON.stringify(config));

        // Attempt to Schedule (Browser Notification)
        // Note: Real "Alarm" requires Service Worker or Native App.
        // For now, we just save it and could trigger local notifs if open.
        if (config.enabled && "Notification" in window) {
            Notification.requestPermission();
        }

        console.log("Alarm Saved:", times);
    },

    // NEW: Init Reminder (Check for pending alarms)
    initReminder: function () {
        // Just a placeholder to prevent crashes if called
