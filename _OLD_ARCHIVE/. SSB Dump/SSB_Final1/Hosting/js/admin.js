        ...
        */
        ...
        sellerKeys.forEach(mobile => {
            ...
            if (mobile === '8851168290') {
                ...
                // Check for KIT3 Ownership
                let isKit3 = false;
                const history = getLocalStorageData(STORAGE_KEYS.PAID_HISTORY, []);
                const userRecord = history.find(h => (h.phone === mobile || h.mobile === mobile));
                if (userRecord && userRecord.kitId === 'KIT3') isKit3 = true;

                if ((salesCount >= 5 || isKit3) && (!smpList[mobile] || smpList[mobile].status !== 'ACTIVE')) {
                    ...
                    if (isKit3) statusBadge += `<br><span style="font-size:0.6rem; color:#aaa;">(Method: Family Kit)</span>`;
                    else statusBadge += `<br><span style="font-size:0.6rem; color:#aaa;">(Method: 5 Sales)</span>`;
                }
            }
...
function renderActiveSMPs() {
    ...
    // FAILSAFE REMOVED: Do not auto-inject Founder
    // if (!list['8851168290'] ... ) { ... }
...
function repairAdminData() {
    ...
    // Founder Check Removed (Production Cleanup)
    /*
