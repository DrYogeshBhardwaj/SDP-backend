...
seederForm.addEventListener('submit', async (e) => {
    e.preventDefault();

    // Capitalize Name Logic
    let rawName = document.getElementById('seeder-name').value;
    ...
    userDocRef.update({
        ...
        role: 'seeder' // UPGRADE USER ROLE TO SEEDER
    }).then(() => {
        alert("Seeder Profile Activated! Welcome to the Partner Program.");
        // UI should update automatically via onSnapshot, but let's force a reload to be sure
        // logic catches up if needed, though updateSeederUI handles it.
        // Actually, let's just let the snapshot handle it.
