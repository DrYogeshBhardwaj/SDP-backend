 ...
 * Business User Dashboard Logic
 * Do not majorly refactor without explicit permission.
 * ==========================================
 */
class DashboardBusinessApp {
    constructor() {
        this.user = null;
        this.identity = null;
        this.init();
    ...
    async loadUserData() {
        try {
            ...
            if (res.ok) {
                ...
                this.identity = this.user; // In Business, user object contains the balances
    ...
    updateUI() {
        ...
        const walletEl = document.getElementById('wallet-cash') || document.getElementById('wallet-display-seeder');
        if (walletEl) {
            const val = this.user.walletCash || this.user.walletBalance || 0;
            walletEl.textContent = walletEl.id === 'wallet-display-seeder' ? `₹${val}` : val;
        }

        this.loadHistory();
        this.checkPartnerStatus();
        this.checkSeederStatus(); 
    ...
    async loadDailyContent() {
        ...
        try {
            ...
            if (res.success && res.data && res.data.announcement) {
                const { announcement, shouldPopup } = res.data;
                
                if (shouldPopup) {
                    this.showAnnouncementPopup(announcement);
                    if (announcement.priority !== 'CRITICAL') {
                        await ApiClient.post(`/announcements/seen/${announcement.id}`);
                    }
                }

                const broadcastEl = document.getElementById('admin-broadcast');
                const broadcastTextEl = document.getElementById('broadcast-text');
                if (broadcastEl && broadcastTextEl) {
                    broadcastTextEl.textContent = announcement.message;
                    broadcastEl.classList.remove('hidden');
                    if (announcement.priority === 'CRITICAL') broadcastEl.style.border = "2px solid #ef4444";
                }
            }
        } catch (e) {
    ...
    showAnnouncementPopup(a) {
        ...
        const modalHtml = `
            <div id="announcement-modal" style="position:fixed; inset:0; background:rgba(15,23,42,0.9); z-index:9999; display:flex; align-items:center; justify-content:center; padding:20px; backdrop-filter:blur(10px);">
                <div style="background:#1e293b; padding:2.5rem; border-radius:32px; max-width:450px; width:100%; text-align:center; color:white; border: ${isCritical ? '4px solid #ef4444' : '1px solid rgba(255,255,255,0.1)'}; box-shadow:0 25px 50px -12px rgba(0,0,0,0.5);">
                    <div style="background:rgba(59,130,246,0.1); color:${isCritical ? '#ef4444' : '#3b82f6'}; width:60px; height:60px; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 1.5rem; font-size:1.5rem;">${isCritical ? '⚠️' : '📢'}</div>
                    <h2 style="margin-top:0; font-size:1.5rem; color:${isCritical ? '#ef4444' : 'white'};">${a.title}</h2>
                    <p style="margin: 1.5rem 0; line-height:1.6; color:#94a3b8;">${a.message}</p>
                    <button onclick="this.closest('#announcement-modal').remove()" style="background:${isCritical ? '#ef4444' : '#3b82f6'}; color:white; border:none; padding:1.25rem 2rem; border-radius:16px; width:100%; font-weight:700; cursor:pointer; transition:0.2s;">Dismiss Notice</button>
                </div>
            </div>
        `;
    ...
    renderSeederStats() {
        ...
        const link = `${basePath}/join_business.html?ref=${identity.id}`;
    ...
    async loadHistory() {
        try {
            const res = await apiRequest('/api/minutes/history');
            const historyList = document.getElementById('recent-history-list');
            if (res.ok && res.data.length > 0) {
                historyList.innerHTML = res.data.map(session => `
                    <li class="flex justify-between items-center py-2 border-b border-gray-100">
                        <span class="text-sm text-slate-600">${new Date(session.createdAt).toLocaleDateString()}</span>
                        <span class="text-sm text-danger font-bold">
                            -${session.duration}m
                        </span>
                    </li>
                `).join('');
    ...
    async requestPayout() {
        try {
            const res = await apiRequest('/api/finance/payout', { method: 'POST' });
            if (res.ok) {
                alert("Payout Requested! Admin will verify.");
                this.loadUserData(); // Refresh wallet
            } else {
                alert(res.error || "Failed to request payout");
            }
        } catch (e) {
            alert(e.message);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        window.dashboardApp = new DashboardBusinessApp();
    } catch (e) {
        console.error("Critical Init Error:", e);
        if (window.onerror) window.onerror("Init Failed: " + e.message, "dashboard-business.js", 0);
    }
});
